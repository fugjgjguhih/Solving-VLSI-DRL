...
def evaluation(V, xes):
    ...
    for i in range(n-1):
        ...
        edge[:,:,1].scatter_(dim=1,index=ht,src=setupper(edge[:,:,1],Xv-yg,vt,if2))        # edge[h][1] = max(edge[V][1], Xv-yg)
        if3 = if2*(Xv <= Xh)
        if4 = if3*(Yh-Yv >=0)
        ...
        edge[:,:,4].scatter_(dim=1,index=vt,src=setupper(edge[:,:,4],Xh,vt,if3))
    for i in range(n):
        length += torch.sum((edge[:,:,1]-edge[:,:,0]),dim=(0,1))
        length += torch.sum((edge[:,:,3]-edge[:,:,2]),dim=(0,1))
        length = length.float()
        length += 1.414*torch.sum(edge[:,:,5]-edge[:,:,4],dim=(0,1))
        length += 1.414*torch.sum((edge[:,:,7]-edge[:,:,6]),dim=(0,1))
    return length
