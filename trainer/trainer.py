...
import os
def test(actor,critic,device,evaluator,vaild_set,n_pin):
    actor =actor.to(device).eval()
    critic =critic.to(device).eval()
    total_cost=0
    cnt =1
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    for v,_ in vaild_set:
        v= v.to(device).reshape(-1,n_pin,2)
        xes,p = actor(v)
        xes =xes[:,1:,:]
        reward = evaluator.eval_batch(v.cpu().detach().numpy(),xes.cpu().detach().numpy())
        reward=torch.tensor(reward, dtype=torch.float)

        total_cost += reward.mean()
        cnt+=1
    print("average_cost:{0}".format(total_cost/cnt))
def vaild(actor,critic,evaluator,vaild_set):
    actor =actor.eval()
    critic =critic.eval()
    total_cost=0
    cnt =1
    ...
    for v in vaild_set:
        ...
        v = v.to(device).reshape(-1,actor.n_pin,2)
        xes,p = actor(v)
        xes =xes[:,1:,:]
        reward = evaluator.eval_batch(v.cpu().detach().numpy(),xes.cpu().detach().numpy())
        reward=torch.tensor(reward, dtype=torch.float)
...
def train(bz,n_pin,accumulation,actor,critic,epoch_number,device,max_norma ,max_normc ,evaluator,model_dir,vaild_set,accuary_reward,lr_a=0.002,lr_c=0.002):
    ...
    for epoch in range(epoch_number):
        ...
        if step % 0.1*epoch_number == 0.1*epoch_number-1:
           acheduler.step()
           ccheduler.step()
           loss=vaild(actor,critic,evaluator,vaild_set)
